...
export default function JobAlert() {
  ...
  const { alerts, isLoading, deleteAlert, isDeleting } = useJobAlert();
  ...
  if (isLoading) {
    return (
      <div className="md:px-6 px-4 dm-font leading-[100%]">
        ...
        <div className="bg-white border-[1px] border-[var(--black-white-200)] rounded-[16px] md:px-6 px-4 py-8">
          <p className="text-center text-gray-500">Loading job alerts...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
        ...
        <div className="bg-white border-[1px] border-[var(--black-white-200)] rounded-[16px] md:px-6 px-4 mb-6">
          <div className="flex flex-col md:flex-row pt-4 md:items-center items-start justify-between">
            <h2 className="text-[20px] font-[400] text-[var(--black-white-900)] pt-2 md:pt-0">
              You have {alerts.length} job alert{alerts.length !== 1 ? "s" : ""}.
            </h2>
            <Btn
              type="button"
          ...
          {alerts.length === 0 ? (
            <div className="py-12 text-center">
              <p className="text-gray-500 mb-4">No job alerts yet</p>
              <Btn
                type="button"
                className="h-[40px] w-[173px] rounded-[50px] text-[16px] text-white bg-[var(--primary-1200)] hover:bg-[#078e63]"
              ...
              {alerts.map((alert) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    {alert.experienceLevel.map((level, idx) => (
                      <div
                        key={idx}
                        className="h-[30px] px-3 bg-[var(--primary-100)] flex justify-center text-[#178261] items-center rounded-[80px] text-[14px]"
                      >
                        {level}
                      </div>
                    ))}
                  ...
                  <div className="flex gap-2 mt-3 flex-wrap">
                    {alert.jobType.map((type, idx) => (
                      <span
                        key={idx}
                        className="h-[30px] px-3 bg-[var(--primary-200)] text-[#178261] text-[14px] rounded-[80px] font-[500] flex items-center justify-center"
                      >
                        {type}
                      </span>
                    ))}
      ...
      <DeleteModal
        ...
        message={`Are you sure you want to delete the "${selectedAlert?.jobTitle}" alert? This action cannot be undone.`}
